...
class Ch007TripletSumToZeroTest {
    ...
    void solution() {
        ...
        assertEquals(expectedJson, c.solution(new int[]{-3, 0, 1, 2, -1, 1, -2}));

        expectedJson = Arrays.asList(
                Arrays.asList(-5, 2, 3),
                Arrays.asList(-2, -1, 3)
        );
        assertEquals(expectedJson, c.solution(new int[]{-5, 2, -1, -2, 3}));

        expectedJson = List.of(
                Arrays.asList(0, 0, 0)
        );
        assertEquals(expectedJson, c.solution(new int[]{0, 0, 0}));
    }
}